...
\label{in-state-op}
Store high-level-language processing \emph{state} in-system even if the language processor \emph{code} remains running on the platform. This will ease porting the processor to in-system implementation and support a gradual transition.
\end{heuristic}
