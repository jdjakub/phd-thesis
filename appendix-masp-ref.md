...
By convention, single-argument functions like `quote` name the argument `to` in order to provide the reading "apply *function* to *argument*". Functions symmetric in their arguments, such as `+` and `-`, number them instead: `apply: +, 1: 10, 2: 20`.
