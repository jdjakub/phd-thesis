...
\paragraph{Use The Right Tool For The Job} is a widespread maxim in programming \cite{RightTool}. The ideal conditions capturing the spirit of this idea are as follows:

* What is "right" is subjectively determined by the programmer, even on a whim. When proposing a change to a language (e.g. Python), every user of the language is forced to confront the change, which invites debate about what is "right" for everyone. This can be sidestepped if each programmer can use what is right for their own context without this being forced on others.
...
We do not expect to inhabit such ideal conditions. Even so, being aware of them lets us get a sense of how applicable this principle is and alerts us to any low-hanging fruit waiting for improvement.

\paragraph{Polyglot Programming} is the practice of using multiple languages in a single project \cite{Polyglot}. Modules implemented in different languages need to share data and invoke each other's functions, for which there are several approaches:
...
* Polyglot programming *within* a shared process address space is trickier; the classic approach is to have different languages (e.g. C, Pascal) compile to a common *object file* format understood by the linker. This is restricted to compile-time, however, so for runtime sharing languages use Foreign Function Interfaces (FFIs). This practice has been critiqued by Kell \cite{MMM} which advocates the use of "integration domains" instead.
* Polyglot programming within a single *file* is rare and restricted to special cases. Perl and JavaScript source code support regexes written with a local syntax. C# supports Language INtegrated Queries (LINQ) which are a C# syntax adaptation of SQL queries. Unlike the freedom to choose any combination of existing languages for an entire project, these instances only permit use of specific sub-languages decided by the designers of Perl, JavaScript and C#.

\paragraph{Domain-Specific Languages (DSLs)} go beyond Polyglot Programming by encouraging *custom* languages designed by the programmer for their problem domain. Where Polyglot Programming is about making the best use of *existing* languages designed by someone else, DSLs come closer to a *freedom* to use what one subjectively determines to be the best tool for one's job. JetBrain's MPS \cite{MPS} is an interactive programming system that encourages DSLs. The COLA design \cite{COLAs} supports "mood-specific languages" intended to span a range of scopes down to individual expressions, and the related OMeta \cite{OMeta} project is a platform for custom DSLs. "Reader Macros" in Lisp allow a programmer to use custom syntax for parts of the code.
...
\joel{
A famous maxim in programming is "use the right tool for the job" and this . Current programming supports a heavily restricted form of this freedom: we may use the right *language* for the job, which for our interest here amounts to the right *syntax.*^[Programming languages differ by semantics as well, but for Notational Freedom we are focused on the surface notation aspects only.]

The "job" also has a lower size limit. While it is easy to use a desired syntax for an entire project, and possible to use different syntaxes for different modules within a project (polyglot programming \cite{Polyglot}), we tend not to see different syntaxes at a smaller scale---say, for different functions or lines of code within a file.

There are minor exceptions to this.  Yet these are still decisions that have been made by language designers and set in stone for all contexts and users. We could call this property *syntactic diversity*, where a language that is *called* A additionally permits syntaxes B, C, and D in certain situations.

This goes a small way towards "use the right tool for the job", but what is conspicuously missing is the ability for the *programmer* to decide which syntaxes to use and where based on the context of the current project. This would be syntactic *freedom* beyond pre-approved diversity.}

## Precursors of Explicit Structure
\paragraph{Structure} is the relation of parts to wholes. An entire data structure is made up of smaller parts with references to each other. From a machine's point of view, a data structure is a graph of memory blocks connected by numerical pointers. For humans, it is a graph of dictionaries with named entries, some of which point at other dictionaries. Both of these models can be visualised as boxes with arrows.

\paragraph{Binary files} are *compacted* data structures. Unlike data structures in memory, which can be sparsely spread across large regions and intermingled with each other, a binary file contains the parts densely packed together without any unrelated data.

\paragraph{Syntax} in programming refers to the "look and feel" of a language's textual source code. Formally, syntax is the set of rules defining legal and illegal symbol sequences. This idea can be metaphorically extended to non-sequential structures. For example, we can think of C `struct` definitions as setting out the valid "shape" of the parts of a data structure. The same applies to binary file formats.

\paragraph{Quantitative Syntax} is a term introduced by \cite{Infra} (page\ 13) for the pattern of prefixing a block with its length and using numerical pointers to link structures. A simple example is the Pascal String which begins with a length byte and continues for that many characters.

\paragraph{Qualitative Syntax}, in contrast, relies on special delimiters. For example, the C String begins right away with its characters, relying on a null byte to show up at some point and signal the end.

\paragraph{Text files a.k.a. Strings} are lists of plain text characters. In programming, they are often containers for machine-readable structures as an alternative to binary files. Like the latter, they compact the structure, but in a way subject to the constraints of plain text and qualitative syntax. In practice, the structure in question is usually a tree, in which case the string is built as an *in-order* traversal. This means that special qualitative syntax (usually the matched bracket characters `(`, `[`, `{`, or `<`) is employed *around* substrings to encode the structure.

\paragraph{Parsing and serialising} convert between strings and structures. Parsing *recovers* structure implicit in a string, while serialising spins out the structure into a string.

\paragraph{Editors} are programs for creating various data structures in the form of files. Editors for 3D models, vector graphics, raster images, audio, and video understand the file formats and strive to save only valid files. It is usually not possible to even *express* a structure in the editor that will cause an error in a consumer of the data. Such cases are exceptional: for example, a 3D scene might open without errors in another 3D editor, but cause errors in a game engine according to the latter's additional requirements---perhaps it expects specific objects in the scene named `Player`, `Exit`, and so on. Nevertheless, for most editors and most use cases, the consumer-side validity rules are in harmony with the producer-side rules.

\paragraph{Text Editors} are a type of editor for plain text files. However, they are widely used to write code in programming languages, which have extra syntax rules beyond the plain text format. Unlike most editors, text editors *can* save files that are invalid from the perspective of their consumers under realistic use-cases. These "syntax errors" are then discovered at the point of consumption.
