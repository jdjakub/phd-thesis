...
Therefore we should not be surprised to see self-sustainability in our native Unix Paradigm. What may be slightly novel is that an individual Unix system is also largely self-sustainable (its "large scope"), and it is interesting to see how this property is lost on the way down to the "small scope".
...
Compiled programming languages like C++ or Java are used via several different Unix processes. These include interactive ones like text editors, and batch-mode ones like the compiler. Self-sustainability ought to be the analogue of *self-hosting* for interactive programming systems.
